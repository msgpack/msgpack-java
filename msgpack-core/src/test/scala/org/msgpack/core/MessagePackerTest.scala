...
class MessagePackerTest extends AirSpec with Benchmark:
  ...
  test("MessagePacker") {
    ...
    test("improve the performance via reset method") {
      ...
      t("buffer-reset").averageWithoutMinMax <= t("no-buffer-reset").averageWithoutMinMax shouldBe
        true
    }
    ...
    test("pack larger string array than byte buf") {
      ...
      val testCases = Seq(32 -> 30, 33 -> 31, 32 -> 31, 34 -> 32)
    ...
    test("pack a lot of String within expected time") {
      ...
      val t =
        time("packString into OutputStream", repeat = 10) {
      ...
      t("file-output-stream").averageWithoutMinMax <
        (t("byte-array-output-stream").averageWithoutMinMax * 5) shouldBe true
    }
  }
  ...
  test("support read-only buffer") {
    ...
    val out     = new ByteArrayOutputStream()
    val packer = MessagePack.newDefaultPacker(out).packBinaryHeader(1).writePayload(payload).close()
  }
  ...
  test("be able to disable STR8 for backward compatibility") {
    val config = new PackerConfig().withStr8FormatSupport(false)
