...
public class ValueBuffer /* TODO implements
            BasicNilValue, BasicBooleanValue, BasicIntegerValue, BasicFloatValue,
            BasicBinaryValue, BasicStringValue, BasicArrayValue, BasicMapValue,
            BasicExtendedValue*/ {
    ...
    public void resetToBinaryValue(byte[] value, int off, int len) {
        type = ValueType.BINARY;
        union.setByteBuffer(ByteBuffer.wrap(value, off, len));
    ...
    public void resetToStringValue(byte[] value, int off, int len) {
        type = ValueType.STRING;
        union.setByteBuffer(ByteBuffer.wrap(value, off, len));
        decodedStringCache = null;
        codingException = null;
    ...
    public void resetToArrayValue(List<Value> value) {
        ...
        union.setList(new ArrayList<Value>(value));
    }

    public void resetToMapValue(Map<Value, Value> value) {
        type = ValueType.MAP;
        union.setMap(new HashMap<Value, Value>(value));
    }
    ...
    public Value immutableCopy(boolean deepCopy) {
        return null;
        /* TODO
        switch (getType()) {
        case NIL:
        ...
        case BOOLEAN:
            return ValueFactory.newBooleanValue(union.getBoolean());
        case INTEGER:
            if (union.getType() == ValueUnion.Type.LONG) {
                return ValueFactory.newIntegerValue(union.getLong());
            } else {
                return ValueFactory.newIntegerValue(union.getBigInteger());
            }
        case FLOAT:
            return ValueFactory.newFloatValue(union.getDouble());
        case STRING:
            synchronized (this) {
                if (union.getType() == ValueUnion.Type.STRING) {
                    return ValueFactory.newStringValue(union.getString());
                } else if (decodedStringCache != null && codingException == null) {
                    if (deepCopy) {
                        return ValueFactory.newStringValue(decodedStringCache);
                    } else {
                        return new StringValueImpl(decodedStringCache, union.getByteBuffer());
                    }
                }
            }
            if (deepCopy) {
                return new RawStringValueImpl(copyByteBuffer());
            } else {
                return new RawStringValueImpl(union.getByteBuffer());
            }
        case BINARY:
            if (deepCopy) {
                return new BinaryValueImpl(copyByteBuffer());
            } else {
                return new RawStringValueImpl(union.getByteBuffer());
            }
        case ARRAY:
            if (deepCopy) {
                ...
                ValueFactory.newArrayValue(union.getList());
            }
        case MAP:
            if (deepCopy) {
                ...
                ValueFactory.newMapValue(union.getMap());
            }
        case EXTENDED:
            // TODO
        }
